...
# Stop training when the loss on the validation data stops improving.
# Save the trained models for later use.
#stop training as the val_loss on the validation stops improve for 10 epochs
# callback = tf.keras.callbacks.EarlyStopping(monitor='val_f1_score',patience=20)
...
for train, val in kfold.split(np.asarray(labels), np.asarray(labels)):
    ...
    cv_count += 1
    print('{}th cv:'.format(cv_count))
    # save the model achieving the best val_f1_score
    callback = tf.keras.callbacks.ModelCheckpoint('bidirection_cv_'+str(cv_cnt)+'.h5', monitor='val_f1_score', \
